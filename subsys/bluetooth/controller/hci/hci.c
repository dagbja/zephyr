...
//	u8_t status;
	...
	if ((u32_t)node_rx->hdr.rx_ftr.extra & BIT(TERM_EVT_CONNECT)) {
		printk("Connection\n");
		sep->status = BT_HCI_ERR_SUCCESS;
	}
	else if ((u32_t)node_rx->hdr.rx_ftr.extra & BIT(TERM_EVT_TIMEOUT)) {
		printk("Duration timeout\n");
		sep->status = BT_HCI_ERR_ADV_TIMEOUT;
	}
	else {
		printk("Limit reached\n");
		sep->status = BT_HCI_ERR_SUCCESS;
	}
	...
	sep->num_completed_ext_adv_evts = 
		(u8_t)((u32_t)node_rx->hdr.rx_ftr.extra & 0xff);

	printk("num_ext_adv_evt = %d\n", sep->num_completed_ext_adv_evts);

	/* Disable adv set, unless event was caused by connection */
//	if (!((u32_t)node_rx->hdr.rx_ftr.extra & 0x2000)) {
#if defined(CONFIG_BT_HCI_MESH_EXT)
//		status = ll_adv_enable(sep->adv_handle, 0, 0, 0, 0, 0, 0);
#else /* !CONFIG_BT_HCI_MESH_EXT */
//		status = ll_adv_enable(sep->adv_handle, 0, 0, 0);
//		LL_ASSERT(status == 0);
#endif /* !CONFIG_BT_HCI_MESH_EXT */
//	}
}
