...
static inline struct ll_adv_set *is_disabled_get(u8_t handle);
		       ...
		       u8_t filter_policy)
{
	...
	if (pdu->type == PDU_ADV_TYPE_DIRECT_IND) {
		...
		if (is_pdu_type_changed) {
			...
			*(u8_t *)&pri_hdr_prev = *(u8_t *)pri_hdr;
		}
		*(u8_t *)pri_hdr = 0;
...
#if defined(CONFIG_BT_CTLR_ADV_EXT)
u8_t ll_adv_data_set(u8_t handle, u8_t len, u8_t const *const data)
{
#else /* !CONFIG_BT_CTLR_ADV_EXT */
...
#if defined(CONFIG_BT_CTLR_ADV_EXT)
u8_t ll_adv_scan_rsp_set(u8_t handle, u8_t len, u8_t const *const data)
{
#else /* !CONFIG_BT_CTLR_ADV_EXT */
...
u8_t ll_adv_enable(u8_t enable)
{
	...
	if (0) {
		...
		if (pri_hdr->adv_addr) {
			...
			sec_dptr = (u8_t *)sec_hdr + sizeof(*sec_hdr);
...
#endif
	{
		u32_t adv_size		= PDU_OVERHEAD_SIZE(phy) +
					  ADVA_SIZE;
		const u16_t conn_ind_us = BYTES2US((PDU_OVERHEAD_SIZE(PHY_1M) +
						    INITA_SIZE + ADVA_SIZE +
						    LLDATA_SIZE),
						   phy);
		const u8_t scan_req_us  = BYTES2US((PDU_OVERHEAD_SIZE(PHY_1M) +
						    SCANA_SIZE + ADVA_SIZE),
						   phy);
		const u16_t scan_rsp_us = BYTES2US((PDU_OVERHEAD_SIZE(PHY_1M) +
...
#endif /* CONFIG_BT_PERIPHERAL */
	{
		...
		if (lll->aux) {
			...
			u32_t ticks_anchor_aux = ticks_anchor + ticks_slot +
...
inline struct ll_adv_set *ull_adv_is_enabled_get(u8_t handle)
{
	struct ll_adv_set *adv;
...
u8_t ull_adv_data_set(struct ll_adv_set *adv, u8_t len, u8_t const *const data)
{
	struct pdu_adv *prev;
	struct pdu_adv *pdu;
	u8_t idx;
...
u8_t ull_scan_rsp_set(struct ll_adv_set *adv, u8_t len, u8_t const *const data)
{
	struct pdu_adv *prev;
	struct pdu_adv *pdu;
	u8_t idx;
		      ...
		      void *param)
{
	...
	if (adv->remain_duration) {
		if (random_delay < adv->remain_duration) {
			adv->remain_duration -= random_delay;
		}
		else {
