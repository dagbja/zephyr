...
static void term_evt_op_stop_cb(u32_t status, void* param);
...
void *ull_event_done(void *param)
{
	...
	if (lll_adv->remain_duration) {
		if (lll_adv->interval < lll_adv->remain_duration) {
			lll_adv->remain_duration -= lll_adv->interval;
		}
		else {
			lll_adv->remain_duration = 1;
		}
	}
#endif /* CONFIG_BT_CTLR_ADV_EXT */
